...
public class WallFilter extends FilterAdapter implements WallFilterMBean {
    ...
    public synchronized void init(DataSourceProxy dataSource) {
        ...
        if (this.dbType == null || this.dbType.trim().length() == 0) {
            LOG.error("this.dbType1:"+this.dbType);
            LOG.error("this.dbType2:"+dataSource.getDbType());
            LOG.error("this.dbType3:"+JdbcUtils.getDbType(dataSource.getRawJdbcUrl(), ""));
        ...
        LOG.error("this.dbType4:"+dbType);
        LOG.error("this.dbType5:"+ (dbType == null));
        if (dbType == null) {
        ...
        } else if (JdbcUtils.IMPALA.equals(dbType)) {
            if (config == null) {
                config = new WallConfig(ImpalaWallProvider.DEFAULT_CONFIG_DIR);
            }

            provider = new ImpalaWallProvider(config);
        } else {
